...
//     await fetch('http://localhost:5000/api/vehicles', vehicle);
//     alert('Vehicle Added!');
//   };

//   const searchVehicles = async () => {
//     const res = await axios.get('http://localhost:5000/api/vehicles/available', { params: search });
//     setResults(res.data);
//   };
...
//       <h2>Add Vehicle</h2>
//       <input placeholder="Name" onChange={e => setVehicle({ ...vehicle, name: e.target.value })} />
//       <input placeholder="Capacity (Kg)" type="number" onChange={e => setVehicle({ ...vehicle, capacityKg: e.target.value })} />
//       <input placeholder="Tyres" type="number" onChange={e => setVehicle({ ...vehicle, tyres: e.target.value })} />
//       <button onClick={addVehicle}>Add</button>

//       <h2>Search & Book</h2>
//       <input placeholder="Capacity Required" onChange={e => setSearch({ ...search, capacityRequired: e.target.value })} />
//       <input placeholder="From Pincode" onChange={e => setSearch({ ...search, fromPincode: e.target.value })} />
//       <input placeholder="To Pincode" onChange={e => setSearch({ ...search, toPincode: e.target.value })} />
//       <input type="datetime-local" onChange={e => setSearch({ ...search, startTime: e.target.value })} />
//       <button onClick={searchVehicles}>Search</button>

//       <ul>
//         {results.map(r => (
//           <li key={r._id}>
//             {r.name} - {r.capacityKg}Kg - {r.tyres} tyres - {r.estimatedRideDurationHours}h
//             <button onClick={() => bookVehicle(r._id)}>Book Now</button>
//           </li>
//         ))}
//       </ul>
//     </div>
